...
def plot_cka_matrix(cka_matrix,doc_len, title="Adjacency matrix",
                    xlabel="Latent Variables", ylabel="Utterance",
                    xlayerids=None, ylayerids=None,
                    every_nth_tick=1, ax=None, fig=None):
    """" Helper Function for Plotting CKA Matrices. """
    d_len=int(doc_len)
    cka_matrix=2*nn.Sigmoid()(cka_matrix[:d_len,:d_len])-1
    cka_matrix=cka_matrix*1.6
    sum=torch.sum(cka_matrix,dim=-1)
    mask=torch.ones_like(cka_matrix)*-999
    
    cka_matrix=torch.where(cka_matrix<0.05,mask,cka_matrix)
    cka_matrix=nn.Softmax(dim=-1)(cka_matrix)
    for i in range(doc_len):
        if sum[i].data==0:
            cka_matrix[i,:]=cka_matrix[i,:]*0
    ...
    if ylayerids is None:
        yticklabels = list(reversed(["U" + str(cka_matrix.shape[0]-i) for i in range(cka_matrix.shape[0])]))
    else:
        assert len(ylayerids) == cka_matrix.shape[0]
        yticklabels = list(reversed([str(i) for i in ylayerids]))
    ...
    if xlayerids is None:
        xticklabels = ["E" + str(i+1) for i in range(cka_matrix.shape[1])]
    else:
