...
from ablation.dagnn import *
from ablation.rank import *
from ablation.gat import*
...
class MyModel(nn.Module):
    def __init__(self,args):
        ...
        self.pred1=Pre_Predictions(args) #情感原因分别一个linear映射，dim->1 但是输入是相同的
    ...
    def forward(self,features,adj,s_mask,s_mask_onehot,lengths,padd_adj,bert_token_b,bert_masks_b,bert_clause_b):
        if self.args.withbert==True:
            ...
            if self.args.withvae==False:
                H=self.gnn(doc_sents_h,adj,s_mask,s_mask_onehot,lengths)  #dagnn
            else:
                H,adj_map=self.vae(doc_sents_h,adj,s_mask,s_mask_onehot,lengths)  #dagnn
            
        else:
            doc_sents_h=features
            if self.args.withvae==False:
                H,adj_map=self.gnn(doc_sents_h,adj,s_mask,s_mask_onehot,lengths)  #dagnn
            else:
                H,adj_map=self.vae(doc_sents_h,adj,s_mask,s_mask_onehot,lengths)  #dagnn
        #print(H.size())
        ...
        return couples_pred, emo_cau_pos, pred1_e, pred1_c,pred2_e, pred2_c,adj_map
    ...
    def loss_pre(self, pred1_e, pred1_c,pred2_e, pred2_c, y_emotions, y_causes, y_mask):
        ...
        return loss_e1+loss_e2, loss_c1+loss_c2
    ...
    def output_util(self, couples_pred, emo_cau_pos, doc_couples, y_mask, test=False):
        ...
        for i in range(batch):
            y_mask_i = y_mask[i]
            max_doc_idx = sum(y_mask_i) #该文档实际的子句个数

            doc_couples_i = doc_couples[i] #实际的情感原因对
            couples_true_i = []
            couples_mask_i = []
            for couple_idx, emo_cau in enumerate(emo_cau_pos): #伪情感因果对
                if emo_cau[0] > max_doc_idx or emo_cau[1] > max_doc_idx: #不存在的句子没必要考虑情感原因对
                    couples_mask_i.append(0)
                    couples_true_i.append(0)
                else:
                    couples_mask_i.append(1) #该pair中的子句索引是真实存在的
                    couples_true_i.append(1 if emo_cau in doc_couples_i else 0) #该pair确实出现在真实的情感原因对中

            couples_pred_i = couples_pred[i]
            doc_couples_pred_i = []
            # if test:
            K=min(20,couples_pred_i.size()[0]) #在20和masked_N*N中选择最小的
            if torch.sum(torch.isnan(couples_pred_i)) > 0:
                k_idx = [0] * K
            else:
                _, k_idx = torch.topk(couples_pred_i, k=K, dim=0) #前K个最大，k_idx是概率较大的pair的索引/位置
            doc_couples_pred_i = [(emo_cau_pos[idx], couples_pred_i[idx].tolist()) for idx in k_idx] 
            #couples_pred_i[idx]：pair是情感原因对的概率 
            #emo_cau_pos[idx]：对应的情感原因对
