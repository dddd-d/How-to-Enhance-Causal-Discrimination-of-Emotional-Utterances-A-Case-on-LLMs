...
parser.add_argument('--gat_feat_dim', type = int, default=192)#gat 输出的维度，因为头为4，所以等于输入的四分之一
parser.add_argument('--feat_dim', type = int, default=768)##模块之间传递的维度
parser.add_argument('--code_dim', type = int, default=192)##vae code 的维度
...
def main(fold_id):
    ...
    #每次只选择1个fold,包含一个训练一个测试一个验证
    train_loader=build_train_data(fold_id=fold_id,batch_size=wandb.config.batch_size,data_type='train',args=args)
    valid_loader = build_inference_data( fold_id=fold_id,batch_size=wandb.config.batch_size,data_type='valid',args=args)
    test_loader = build_inference_data( fold_id=fold_id,batch_size=wandb.config.batch_size,data_type='test',args=args)
    model=MyModel(args)
...
if __name__ == '__main__':
    ...
    for fold_id in range(1,11):
        wandb_config=dict(lr=args.lr,batch_size=args.batch_size)
        wandb.init(config=wandb_config,reinit=True,project='dagnn_test_{}'.format(fold_id))
